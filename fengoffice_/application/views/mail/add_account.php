...
<div class="adminMainBlock">
	...
	<fieldset id="<?php echo $genid ?>incoming_settings_div">
		...
		<div class="mail-account-item">
			<label for="<?php echo $genid ?>method"><?php echo lang('email connection method')?></label><?php
				...
				echo select_box('mailAccount[is_imap]', $options, array("onchange" => $onchange, 'tabindex' => '145', 'id' => $genid . 'method'));
		...
		<div class="mail-account-item" id="<?php echo $genid ?>sslportdiv" <?php if (!array_var($mailAccount_data, 'incoming_ssl')) echo 'style="display:none"'; ?>>
			...
			<?php echo text_field('mailAccount[incoming_ssl_port]', array_var($mailAccount_data, 'incoming_ssl_port', 995), array('id' => $genid.'sslport', 'tabindex'=>'160')) ?>
		</div>
	...
	<fieldset id="<?php echo $genid ?>smtp_settings_div">
		...
		<div class="mail-account-item">
			...
			<?php echo text_field('mailAccount[smtp_server]', array_var($mailAccount_data, 'smtp_server'), array('id' => 'mailSmtpServer', 'tabindex'=>'80')) ?>
		</div>
		...
		<div class="mail-account-item">
			...
			<?php echo text_field('mailAccount[smtp_port]', array_var($mailAccount_data, 'smtp_port',25), array('id' => 'mailSmtpPort', 'tabindex'=>'90')) ?>
		</div>
		...
		<div class="mail-account-item">
			...
			echo select_box('mailAccount[smtp_use_auth]', $options, array(
				'id' => 'mailSmtpUseAuth', 'tabindex'=>'100',
		...
		<div class="mail-account-item" id = 'smtp_specific_auth' style='<?php if(array_var($mailAccount_data, 'smtp_use_auth',1)!=2) echo 'display:none';?>'>
			<div class="mail-account-item">
				...
				<?php echo text_field('mailAccount[smtp_username]', array_var($mailAccount_data, 'smtp_username'), array('id' => 'mailSmtpUsername', 'tabindex'=>'110')) ?>
			</div>
			...
			<div class="mail-account-item">
				...
				<?php echo password_field('mailAccount[smtp_password]', array_var($mailAccount_data, 'smtp_password'), array('id' => 'mailSmtpPassword', 'tabindex'=>'120')) ?>
			</div>
		</div>
	...
	<fieldset id="<?php echo $genid ?>other_settings_div">
		...
		<div class="mail-account-item">
			...
			<?php echo yes_no_widget('mailAccount[is_default]', $genid.'is_default', array_var($mailAccount_data, 'is_default', 0) > 0, lang('yes'), lang('no'), 55) ?>
		</div>
		...
		<div>
			...
			<?php $del_from_server = array_var($mailAccount_data, 'del_from_server', -1) ?>
			<?php echo yes_no_widget('mailAccount[del_mails_from_server]', 'mailAccountDelMailFromServer', $del_from_server >= 0, lang('yes'), lang('no'), 60) ?>
			<?php echo '<span style="margin-left: 10px">' . lang('after') . '</span>'?>
			<?php echo text_field('mailAccount[del_from_server]', $del_from_server < 0 ? 0 : $del_from_server, array('id' => 'mailAccountDelFromServer', 'tabindex'=>'70', 'style'=>'width:25px')) ?>
			<?php echo lang('days'); ?>
		</div>
		...
		<div>
		    ...
		    <?php echo textarea_field('mailAccount[signature]', array_var($mailAccount_data, 'signature'), array('id' => 'signature', 'tabindex'=>'290', 'style' => 'width:100%;max-width:500px;height:100px;')) ?>
		</div>
	</fieldset>
