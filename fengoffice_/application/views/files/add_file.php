...
<div class="coInputTitle"><table style="width:535px"><tr><td><?php echo $file->isNew() ? lang('upload file') : (isset($checkin) ? lang('checkin file') : lang('edit file properties')) ?>
	</td><td style="text-align:right"><?php echo submit_button($file->isNew() ? lang('add file') : (isset($checkin) ? lang('checkin file') : lang('save changes')),'s',array("onclick" => 'og.fileCheckSubmit(\'' . $genid .'\')', 'style'=>'margin-top:0px;margin-left:10px','id' => $genid.'add_file_submit1', 'tabindex' => '210')) ?></td></tr></table>
	</div>
	</div>
	<?php if ($enableUpload) { 
		if ($file->isNew()) {?>
    ...
    <div id="<?php echo $genid ?>selectFileControlDiv">
        ...
        <?php echo file_field('file_file', null, array('id' => $genid . 'fileFormFile', 'class' => 'title', 'size' => '88', 'tabindex' => '10', 'onchange' => 'javascript:og.updateFileName(\'' . $genid .  '\');og.checkFileName(\'' . $genid .  '\')')) ?>
    ...
    <div id="<?php echo $genid ?>addFileFilename" style="<?php echo $file->isNew()? 'display:none' : '' ?>">
        ...
        <?php echo text_field('file[name]',$file->getFilename(), array("id" => $genid .'fileFormFilename', 'tabindex' => '20',  'onchange' => ('javascript:og.checkFileName(\'' . $genid .  '\')'))) ?>
		<br/>
    </div>
  ...
  <?php if($file->isNew()) { //----------------------------------------------------ADD   ?>
   ...
   <?php }  else {//----------------------------------------------------------------EDIT?>
    ...
    <?php if (!isset($checkin)) {?>
      ...
      <div class="header">
    	<?php echo checkbox_field('file[update_file]', array_var($file_data, 'update_file'), array('class' => 'checkbox', 'id' => $genid . 'fileFormUpdateFile', 'tabindex' => '60', 'onclick' => 'App.modules.addFileForm.updateFileClick(\'' . $genid .'\')')) ?> <?php echo label_tag(lang('update file'), $genid .'fileFormUpdateFile', false, array('class' => 'checkbox'), '') ?>
      </div>
      ...
      <div id="<?php echo $genid ?>updateFileForm"  style="<?php echo isset($checkin) ? '': 'display:none' ?>">
        ...
        <div>
          ...
          <?php echo file_field('file_file', null, array('id' => $genid.'fileFormFile', 'tabindex' => '65')) ?>
        </div>
        <div id="<?php echo $genid ?>revisionControls">
          <div>
            <?php echo checkbox_field('file[version_file_change]', array_var($file_data, 'version_file_change', true), array('id' => $genid.'fileFormVersionChange', 'class' => 'checkbox', 'tabindex' => '70', 'onclick' => 'App.modules.addFileForm.versionFileChangeClick(\'' . $genid .'\')')) ?> <?php echo label_tag(lang('version file change'), 'fileFormVersionChange', false, array('class' => 'checkbox'), '') ?>
          </div>
          <div id="<?php echo $genid ?>fileFormRevisionCommentBlock">
            <?php echo label_tag(lang('revision comment'), $genid.'fileFormRevisionComment') ?>
            <?php echo textarea_field('file[revision_comment]', array_var($file_data, 'revision_comment'), array('class' => 'short', 'tabindex' => '75')) ?>
          </div>
        </div>
      </div>
    <?php if (!isset($checkin)) {?>
...
<?php } // if enableupload ?> 
 ...
 <div id="<?php echo $genid ?>add_subscribers_div" style="display:none">
  ...
  if (!$file->isNew())  //Edit file
	  ...
	  echo submit_button(lang('checkin file'),'s',array("id" => $genid.'add_file_submit2', 'tabindex' => '200', "onclick" => 'og.fileCheckSubmit(\'' . $genid .'\')'));
	else
	  echo submit_button(lang('save changes'),'s',array("id" => $genid.'add_file_submit2', 'tabindex' => '200', "onclick" => 'og.fileCheckSubmit(\'' . $genid .'\')'));
  else //New file
    echo submit_button(lang('add file'),'s',array("id" => $genid.'add_file_submit2', 'tabindex' => '200', "onclick" => 'og.fileCheckSubmit(\'' . $genid .'\')'));
  ?>
  </div>
...
<script type="text/javascript">
	Ext.get('<?php echo $genid ?>fileFormFile').focus();
</script>