...
INSERT INTO `<?php echo $table_prefix ?>object_types` (`name`,`handler_class`,`table_name`,`type`,`icon`,`plugin_id`) VALUES
 ...
 ('user', 'Contacts', 'contacts', 'dimension_object', 'user', (SELECT `id` FROM `<?php echo $table_prefix ?>plugins` WHERE `name`='core_dimensions')),
...
INSERT INTO `<?php echo $table_prefix ?>dimension_object_type_hierarchies` (`dimension_id`, `parent_object_type_id`, `child_object_type_id`) VALUES
 ...
 ((SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users'),
 (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='company'),
 (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='user')),
 ((SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users'),
 (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='user'),
...
INSERT INTO `<?php echo $table_prefix ?>members` (`dimension_id`,`object_type_id`, `parent_member_id`, `depth`, `name`, `object_id` )
 SELECT 
 	...
 	0, 1, `o`.`name`, `o`.`id`
 FROM `<?php echo $table_prefix ?>objects` `o` INNER JOIN `<?php echo $table_prefix ?>contacts` `c` ON `c`.`object_id`=`o`.`id`
 WHERE `c`.`is_company`=1 AND (SELECT COUNT(`x`.`object_id`) FROM `<?php echo $table_prefix ?>contacts` `x` WHERE `x`.`company_id`=`o`.`id` AND `x`.`user_type`!=0) > 0
 ON DUPLICATE KEY UPDATE dimension_id=dimension_id;
...
INSERT INTO `<?php echo $table_prefix ?>members` (`dimension_id`,`object_type_id`, `parent_member_id`, `depth`, `name`, `object_id` )
 SELECT 
 	(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users'),
 	(SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='user'),
 	(SELECT `m`.`id` FROM `<?php echo $table_prefix ?>members` `m` WHERE `m`.`object_id` = `c`.`company_id` AND `m`.`object_type_id` = (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='company') AND `m`.`dimension_id`=(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users') LIMIT 1),
 	(SELECT `m`.`depth`+1 FROM `<?php echo $table_prefix ?>members` `m` WHERE `m`.`object_id` = `c`.`company_id` AND `m`.`object_type_id` = (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='company') AND `m`.`dimension_id`=(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users') LIMIT 1),
 	`o`.`name`, `o`.`id`
 FROM `<?php echo $table_prefix ?>objects` `o` INNER JOIN `<?php echo $table_prefix ?>contacts` `c` ON `c`.`object_id`=`o`.`id`
 WHERE `c`.`is_company`=0
 ON DUPLICATE KEY UPDATE dimension_id=dimension_id;
...
INSERT INTO `<?php echo $table_prefix ?>members` (`dimension_id`,`object_type_id`, `parent_member_id`, `depth`, `name`, `object_id` )
 SELECT 
 	(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users'),
 	(SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='workspace'),
 	(SELECT `m`.`id` FROM `<?php echo $table_prefix ?>members` `m` WHERE `m`.`object_id` = `c`.`object_id` AND `m`.`object_type_id` = (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='user') AND `m`.`dimension_id`=(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users') LIMIT 1),
 	(SELECT `m`.`depth`+1 FROM `<?php echo $table_prefix ?>members` `m` WHERE `m`.`object_id` = `c`.`object_id` AND `m`.`object_type_id` = (SELECT `id` FROM `<?php echo $table_prefix ?>object_types` WHERE `name`='user') AND `m`.`dimension_id`=(SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code`='feng_users') LIMIT 1),
 	'My Stuff', 0
 FROM `<?php echo $table_prefix ?>objects` `o` INNER JOIN `<?php echo $table_prefix ?>contacts` `c` ON `c`.`object_id`=`o`.`id`
 WHERE `c`.`is_company`=0
 ON DUPLICATE KEY UPDATE dimension_id=dimension_id;
...
INSERT INTO `<?php echo $table_prefix ?>contact_member_permissions` (`permission_group_id`, `member_id`, `object_type_id`, `can_write`, `can_delete`)
 ...
 WHERE `c`.`is_company`=0 
 	AND `c`.`user_type`!=0 
	AND `ot`.`type` IN ('content_object', 'located', 'comment')
 	AND `m`.`dimension_id` IN (SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code` = 'feng_users')
 ON DUPLICATE KEY UPDATE member_id=member_id;
...
INSERT INTO `<?php echo $table_prefix ?>object_members` (`member_id`, `object_id`)
 ...
 WHERE (`o`.`created_by_id` = `m`.`object_id` OR `o`.`updated_by_id` = `m`.`object_id` OR `o`.`trashed_by_id` = `m`.`object_id` OR `o`.`archived_by_id` = `m`.`object_id`)
	AND `m`.`id` IN (SELECT `id` FROM `<?php echo $table_prefix ?>members` WHERE `dimension_id` IN (SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code` IN ('feng_persons','feng_users')))
 ON DUPLICATE KEY UPDATE member_id=member_id;

INSERT INTO `<?php echo $table_prefix ?>object_members` (`member_id`, `object_id`)
 SELECT `m`.`id`, `o`.`id`
 FROM `<?php echo $table_prefix ?>objects` `o` INNER JOIN `<?php echo $table_prefix ?>members` `m`
 WHERE `m`.`id` IN (SELECT `id` FROM `<?php echo $table_prefix ?>members` WHERE `dimension_id` IN (SELECT `id` FROM `<?php echo $table_prefix ?>dimensions` WHERE `code` IN ('feng_persons','feng_users')))
