<?php return array(
	...
	'administration tool desc test_mail_settings' => 'この簡単なツールを使用して、OpenGooのメーラが動作するかどうか確認するためのテストメールを送信できます。',
	...
	'config category desc general' => 'OpenGooの一般的な設定です。',
	'config category name mailing' => 'メールの設定',
	'config category desc mailing' => 'この設定でOpenGooが使用するメールの送信方法を指定してください。 php.iniの設定のオプションに任せるか、それと異なる他のSMTPサーバを使用するようにここで設定することもできます。',
	'config category name modules' => 'モジュール',
	'config category desc modules' => 'この設定でOpenGooが使用するモジュールを有効または無効を指定してください。無効に設定するとグラフィカルなインターフェイスからは隠れますが、ユーザに設定した作成・編集等の権限は削除されません。',
	...
	'config option desc theme' => 'テーマを使用して、OpenGooのデザインを変更することができます。',
	...
	'config option desc upgrade_check_enabled' => '「はい」を選択すると、ダウンロードできるOpenGooの新しいバージョンの有無をシステムは1日に一度確認します。',
	...
	'backup process desc' => 'バックアップは、アプリケーション全体の現在の状態を圧縮したフォルダに保存します。インストールしたOpenGooを感嘆にバックアップするのに使用できます。<br/>データベースとファイルシステムのバックアップの生成には、平均で数秒程度で済みますので、バックアップの作成はこれらの段階で行います:<br/>1. - バックアップ処理を開始,<br/>2. - バックアップをダウンロード。<br/>3. - オプションで、バックアップは手動で削除できますが、将来は利用できなくなります。<br/>',
	...
	'cron events info' => 'Cronイベントを使用すると、システムにログインせずにOpenGooのタスクを定期的に実行させることができます。Cronイベントを有効にするには、OpenGooのディレクトリ直下にある"cron.php"ファイルを定期的にcronのジョブとして実行するように設定しなければなりません。cronのジョブを実行する周期は、これらのcronのイベントをどの程度の細かさで実行するかを決めることになります。例えば、5分毎にcronのジョブを設定して、アップグレードの有無のチェックを1分毎に確認するようにしたなら、実際のチェックは5分毎に実行されます。cronのジョブを設定する方法について学ぶには、システム管理者かホスティング・サービスの提供者に問い合わせてください。',
	...
	'cron event desc check_upgrade' => 'このCronイベントはOpenGooの新しいバージョンを確認します。',
	...
	'cron event desc send_notifications_through_cron' => 'このイベントを有効にすると、メールによる通知はOpenGooで生成されたときではなく、cronを通して送るようになります。',
	'cron event name backup' => 'OpenGooのバックアップ',
	'cron event desc backup' => 'このイベントを有効にすると、OpenGooのバックアップを定期的に行います。インストールした所有者は、バックアップを管理パネルを通してダウンロードできます。OpenGooのバックアップはZIPファイルとして\'tmp/backup\'ディレクトリに保管されます。',
